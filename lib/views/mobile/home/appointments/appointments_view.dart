...
class _AppointmentsViewState extends State<AppointmentsView> {
  ...
  Future<void> _fetchForRange(DateTimeRange range) async {
    setState(() => _loading = true);
    final results = await HomeCtrl.to.fetchAppointmentsForRange(range.start, range.end);
    if (mounted) setState(() { _fetched = results; _loading = false; });
  }
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    const Icon(Icons.close_rounded,
                                        size: 14, color: DrColors.primary),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Container(
                    width: 1, height: 22, color: DrColors.border),
                Expanded(
                  child: _ActionButton(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          children: [
            Icon(icon, size: 28, color: selected ? color : DrColors.textTertiary),
            const SizedBox(height: 6),
            Text(
              label,
              style: GoogleFonts.inter(
